...
chapter, which variant do you expect to be useful in more situations? Which one is
more efficient?
*/
//my code...
...
when given [1, 2, 3] as argument, and write a listToArray function that produces an
array from a list. Also write the helper functions prepend, which takes an element
...
function arrayToList(array) {
    ...
    for(var i = array.length - 1; i >= 0; i--) {
        if(i === array.length - 1) {
          currentList = {value: array[i], rest: null }
        }else {
            currentList = {value: array[i], rest: currentList}
        }
    }
    return currentList;
};
...
function deepEqual(a, b) {
  if(a === b) return true;
    ...
    if(propsInA == propsInB) return true;
}
